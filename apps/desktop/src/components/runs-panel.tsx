  ...
  return (
    <UI.Card>
      ...
      <div className="desktop-row">
        <UI.Button variant="outline" onClick={() => clearEvents()} disabled={events.length === 0}>
          Clear Feed
        </UI.Button>
      </div>
      <div className="run-feed">
        {events.length === 0 ? (
          ...
          events.map((event) => (
            <article key={`${event.runId}-${event.occurredAt}-${event.status}`} className="run-feed__item">
              <div className="run-feed__meta">
                <span className={`run-feed__status run-feed__status--${event.status}`}>
                  {event.status}
                </span>
                <span>{event.occurredAt}</span>
              </div>
              <p className="run-feed__text">Run: {event.runId}</p>
              <p className="run-feed__text">Correlation: {event.correlationId}</p>
              {event.skillId ? <p className="run-feed__text">Skill: {event.skillId}</p> : null}
